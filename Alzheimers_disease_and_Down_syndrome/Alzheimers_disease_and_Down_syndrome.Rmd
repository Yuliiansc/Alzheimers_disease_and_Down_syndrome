...
common_samples <- intersect(colnames(expression_data), filtered_metadata$Sample_ID)

# Retain only the common samples in both tables
expression_data <- expression_data[, common_samples]
filtered_metadata <- filtered_metadata[filtered_metadata$Sample_ID %in% common_samples, ]
...
# Now check the number of columns (samples) in the loaded data
...
```
```{r}
# Find the common sample names between the expression data and the metadata
common_samples <- intersect(colnames(expression_data), filtered_metadata$Sample_ID)
...
# Set the row names to gene identifiers and remove the gene ID column from the expression data
rownames(expression_data) <- expression_data$ENSG00000160072
...
# Set gene identifiers as row names and remove the first column
rownames(expression_data) <- expression_data$ENSG00000160072  # Assuming 'ENSG00000160072' is the gene ID column
expression_data <- expression_data[, -1] 
